...
产品需要对齐 Codex Sites 的基础站点管理体验：站点具有独立的展示名称，访问 URL 可以改名；名称与 slug 可以分别修改。修改 slug 不应重新部署内容，旧 URL 应停止路由并在安全清理后释放，同时站点身份、权限、版本、runtime config 和 runtime data 必须连续。

本设计是一项纵向能力：虽然涉及 pages-api、pages-router、kv-gateway、pages-console 和 CLI 兼容，但它们共同完成一次站点元数据修改，不拆成彼此独立的产品功能。

参考行为：[Codex Sites — Change a Site URL](https://learn.chatgpt.com/docs/sites#change-a-site-url)。本项目只对齐“显示名称与 URL 改名”能力；旧 URL 不做兼容跳转，以便 slug 可重新使用。
...
- 新 URL 在修改成功时可访问；旧 URL 停止路由，旧 slug 在 pointer 清理确认和现有 5 分钟 reuse hold 后可重新使用。
...
| 待释放地址 | hostname claim | pointer 清理后短暂 hold | 防止旧路由残留期间被其它站点接管 |
...
- `slug_revision INTEGER NOT NULL DEFAULT 1`：只在 canonical slug 变化时递增，用于路由收敛。
- `slug_routing_synced_revision INTEGER NOT NULL DEFAULT 1`：新 canonical pointer 已确认且旧 pointer 已清理的 slug revision。
...
- 旧 hostname 的 claim 原子转为 `site_slug_renamed_pending_cleanup` hold；在旧 route pointer 确认删除前不允许其它站点接管。
- pointer 清理确认后，旧 claim 进入现有 5 分钟 reuse hold；到期后可被任意有权限的站点重新获取。
- 本 site 在 hold 期间改回原 slug 时可直接重新激活自己的 claim。
- 其它 site、v1 资源或尚处于 reuse hold 的 claim 一律返回冲突，不透露占用方。
- 删除站点时，把该 site 当前及尚待清理的 hostname claim 全部纳入现有 hold/release 生命周期。

## 应用边界

新增聚焦的 `updateSiteMetadata` application use case，校验并部分更新 `title` / `slug`，协调 hostname claim、route pointer 和审计。
...
- 目标 slug 若属于其它 live site、v1 claim 或仍在 reuse hold，统一返回 `SITE_SLUG_CONFLICT`。
- 本 site 改回自己仍在 hold 的旧 slug 合法。
- 单次 metadata 请求同时包含 `title` 与 `slug` 时，D1 变更在一个事务中提交；Console 为满足独立编辑体验，每个控件只发送自身字段。

### 一致性流程

slug 修改复用现有 site commit lease，与 deploy、rollback、delete 和访问策略变更串行。锁内重新读取 site、route、active version、ACL 和待清理 hostname claims，并使用 fencing token / expected route tuple 防止旧写覆盖新状态。

D1 batch 一次完成：

1. 获取或建立目标 hostname claim。
2. 将旧 canonical hostname claim 转为无到期时间的 pending-cleanup hold；若目标是本 site 的旧 claim，则先将其恢复为 active。
...
5. 将 `sites.slug_routing_synced_revision` 保持为旧值，表示新 pointer 写入和旧 pointer 清理尚未共同确认。
...
2. 清理属于该 site 的全部 pending-cleanup 旧 hostname pointer，并确认 pointer 不存在。
3. 每个旧 pointer 清理确认后，把对应 claim 改为带 5 分钟 `reuse_hold_until` 的普通 hold。
4. canonical pointer 已确认且没有 pending-cleanup claim 后，以 slug revision CAS 把 `sites.slug_routing_synced_revision` 更新为当前 `slug_revision`；旧 reconcile 不得把更新后的 rename 错标为 ready。

`routingStatus` 由 `slug_routing_synced_revision === slug_revision` 推导为 `ready`，否则为 `pending`。只有新 canonical pointer 写入和全部旧 pointer 清理都确认后，含 slug 的 mutation 才返回 `200`。如果 D1 已提交但 KV 同步未完全成功，返回 `202`，并使用 `ctx.waitUntil` 重试；现有每 15 分钟 scheduled handler 增加有界 reconciliation，处理 revision 未收敛的站点。客户端以同一目标 slug 重试时也必须执行 repair，而不能因字段同值直接 no-op。title-only 请求不被既有 pending rename 阻塞：title 提交成功时返回 `200`，但返回的 site 仍如实携带 `routingStatus: "pending"`。

这种顺序允许 KV 最终一致窗口内短暂出现“旧地址仍服务原内容”或“新地址尚未可用”，但旧 claim 在清理完成前不会被其它站点接管。API 不得在新旧 pointer 状态未确认时返回已完全生效。reconcile 只写当前 D1 authority 指向的 hostname、只清理仍属于该 site 的旧 hostname，并保持幂等。
...
Router 只服务 canonical serve snapshot，不新增 redirect 类型或 308 分支。slug 改名后旧 pointer 被清理，旧 URL 返回既有的未找到响应；reuse hold 到期并被其它站点获取后，旧 URL 才可能服务新站点。

旧 pointer 删除采用 hostname 级串行化。清理请求同时携带不可变 `siteId`、`routeId` 和本次 rename 的 route generation/policy version；hostname DO 必须读取 pointer 指向的 snapshot，并确认 environment、hostname、`siteId`、`routeId` 都匹配，且 pointer tuple 不高于预期，才能删除。pointer 缺失视为已清理；身份或 tuple 不匹配则 fail closed 并保持 claim pending-cleanup，由 reconciliation 重试。

pending-cleanup claim 以改名时的 `released_at` 与 `slugRevision` 共同作为 cleanup token。每次外部 pointer 操作后，D1 CAS 必须重新确认 site commit lease、当前 slug revision、claim owner/status/token，以及该 hostname 已不是当前 canonical，才可把 claim 转入 5 分钟 reuse hold。reconciler 使用同一 site commit lease；租约过期、连续改名或回切导致 authority 改变时，本轮不得释放 claim或标记 routing ready。只有删除确认及上述 CAS 成功后才开始 reuse hold，避免旧 route 尚存时被其它站点接管。
...
| `SITE_SLUG_CONFLICT` | 409 | 当前 slug、v1 claim 或 reuse hold 冲突 |
...
- URL 保存前展示新 hostname，并明确“旧地址将停止访问并在安全期后释放；本地 `xd-cell.config.json.name` 需同步更新”。
...
- 运行配置的变量/Secret 保存或替换后保留当前列表并后台刷新，不把整块内容切换为 loading 占位；弹窗打开时预留滚动条槽，避免页面横向跳动。

## CLI slug 改名提示

`xd-cell.config.json.name` 继续表示 slug，不做自动重命名。旧 slug 不再解析到原 site；用户必须在 Console/API 改名后同步修改本地 `name`。reuse hold 期间使用旧 slug 发布会按现有 hostname claim 冲突失败；hold 到期后该 slug 可被重新使用，因此 CLI 不应把它视为原站点身份。

Console 在确认改名前明确提示该后果。CLI 的站点定位和错误提示继续只使用当前 canonical slug，避免以历史名称访问或更新错误站点。
...
3. 发布能读取 v2/v3/v4 serve snapshot 的 Router；此时 pages-api 仍只写旧 serve snapshot。
4. 部署 pages-api 兼容基线：新建站点写 `data_namespace`，所有 snapshot 写入都携带 namespace，metadata mutation feature flag 仍关闭。
5. 部署 pages-console，先在 staging 完成端到端验证，再手动触发 production workflow。
...
- A→B 后 A 不跳转；pointer 清理前其它 site 不能占用 A，清理后经过 5 分钟 hold 可复用；本 site 在 hold 内可改回 A。
- 并发 rename/create/deploy/delete 由 claim、lease 和 fencing 阻止丢失更新。
- snapshot 部分失败返回 202，同值重试与 scheduled reconciliation 可收敛。
- canonical 未同步、旧 pointer 未清理和 stale reconcile CAS 均能正确维持 `routingStatus=pending`；全部操作确认后 list/detail 变为 ready。
- 删除站点 hold 当前及待清理 claim，并清理相关 route pointer。

### Router 与 Gateway 测试

- Router 继续读取 v2/v3 serve snapshot，并读取带 `dataNamespace` 的 v4 serve snapshot。
- 旧 hostname pointer 清理后返回未找到，不产生 3xx，也不执行 User Worker 或签发 runtime capability。
- 旧 claim 在 pointer 确认清理前保持不可复用，避免跨站点路由。
...
- UI 覆盖 displayName fallback、两个独立 pending/error 状态、202 提示，以及运行配置后台刷新不卸载列表。
- CLI 不把历史 slug 解析到原 site；Console 清楚提示同步本地配置。
- config inventory、workflow 和 docs tests 覆盖 feature flag 与两个环境的隔离。

最后运行相关 focused `node:test`、`pnpm lint` 和 `pnpm test`。staging 验收至少包括：连续两次 rename、旧地址不跳转且最终可复用、当前地址内容、访问策略、deploy/rollback、runtime data 改名前后读写、变量保存无页面跳动，以及 Admin/Workspace 权限矩阵。

## 完成标准

- 用户可在 Public API、Workspace Console 和 Admin Console 分别修改 title 与 slug。
- 两个字段可独立保存，API 与 UI 对空值、格式、权限和冲突的行为一致。
- 活跃站点 rename 不产生 deployment/version；旧 hostname 不跳转，pointer 清理确认和 5 分钟 hold 后可复用。

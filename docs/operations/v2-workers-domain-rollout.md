...
- `normalized_slug` 不是唯一 key，而是同名站点释放组。存量 v1 / v2 如果已经分别占用不同 hostname，例如 `foo.workers.xd.team` 与 `foo.pages.xd.team`，回填时允许同时写入 active claim，保证两边原 host 都能继续 deploy / rollback / policy update。
- 新增 claim 仍必须检查同一 `environment + normalized_slug` 下是否存在他方 live claim。也就是说，存量同 slug 可以共存，但不能新增第三个 owner，也不能把存量 `pages.xd.team` 站点静默迁移到 `workers.xd.team`。只有同 slug 组内所有 claim 都 release / delete，且 reuse hold 到期后，该 slug 才重新可用。
...
- v1 和 v2 已有 live 数据且 `normalized_slug` 相同、但 hostname 不同，是 legacy coexistence，不写入 `hostname_claim_conflicts`，也不阻断回填 apply。回填 summary 中的 `slugCoexistence` 只做上线前人工复核和抽样验证。
...
- 新增一次性回填脚本或内部管理命令：
  ...
  - 发现同 `normalized_slug`、不同 hostname 的存量 v1/v2 候选时，写入两条 hostname claim，并在 summary 中增加 `slugCoexistence` 计数；这类共存不阻断 apply。
...
- [ ] 回填脚本读取脱敏 v1 `SITES` 导出和 v2 未删除站点对应 `site_routes` 导出，生成 insert-if-hostname-absent SQL 和冲突 SQL；同 hostname 多 owner 默认 fail closed，同 slug 不同 hostname 记为 `slugCoexistence` 不阻断 apply。
...
- [ ] production migration + 回填前确认使用 production KV/D1；dry-run 后 blocking 冲突数量为 0，`slugCoexistence` 已人工复核；再用 `apply=true` 写入 D1。

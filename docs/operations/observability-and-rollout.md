...
| deployment cleanup task     | `deployment_resource_cleanup_tasks` 是 WFP user Worker、v1 Worker script 和 v1 `SITES` KV 延迟 GC 的真相源；Cron Trigger 小批量处理到期 `pending` / `failed` task，Admin Console 可查看并手动 run；v1 Worker 删除前必须从 site/environment 重新推导脚本名并确认 Cloudflare 完整 route 列表无引用 |
...
- `Deployment Cleanups > Orphan Scan` 由管理员手动触发，只扫描当前环境 WFP dispatch namespace 的受管脚本，并与 active route、`artifact_availability='active'` 的版本和未完成 cleanup task 对账。响应字段为 `referencedByActiveRoute`、`rollbackEligibleVersion`（仅统计非当前 route 目标、但 artifact 仍 active 的旧版 Worker，当前活跃版本不计入）、`hasPendingCleanupTask` 和 `orphanCandidate`；`orphanReason` 限定为 `no_d1_reference`、`deleted_site`、`stale_previous_version`。这些分类不等同于“可以删除”，也不固化版本保留策略。扫描会先用 namespace `script_count` 校验上游清单完整性；只有 `completeness=complete` 的结果才允许进入回收 backfill，`incomplete` 只能查看并重试。
  上游 dispatch scripts 清单端点目前没有正式 API reference / SDK 分页契约，线上实测为 KV 同款 cursor 分页（`result_info` 只含 `count` / `cursor`，空 cursor 即终页）。客户端仅在 `result_info` 完全缺失时把本页视为 undocumented single-page 候选；`result_info` 存在时必须匹配三种合法形态之一——原生正整数 `page` + `total_pages`、`page` + `per_page`/`total_count`、或 `count`/`cursor`——否则 fail-closed；cursor 翻页拒绝重复 cursor、空页续传和超出上限的页数。随后读取 namespace 详情中的 `script_count` 做完整性校验；每条清单记录也必须解析出非空 Worker name。首次数量不一致会重试一次，仍不一致时返回 `completeness: 'incomplete'`，同时保留 `scannedCount` 和 `namespaceScriptCount`，Console 必须显示警告。未来删除阶段只能基于 `completeness: 'complete'` 的扫描结果做删除决策；不完整扫描不得触发回收。
- `Legacy v1 Sites` 通过 `pages-api` 直接读取当前环境的 v1 SITES KV 和 account-level Workers，再与未删除的 v2 同名站点对账。页面展示站点名称、URL、preset、网络限制、更新时间、对应 v1 Worker、迁移候选标记和归属 Token（legacy API 已墓碑化，该 token 不再具备任何操作能力，仅作平台管理员可见的归属标记）；KV metadata 中的其它内部字段不进入响应，token 也不得写入审计或日志。

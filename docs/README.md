...
| agent 协作规范                    | `AGENTS.md`、`.agents/agent-behavior-guidelines.md`                                                                                                       | Codex、Claude 和其它 coding agent 的项目规则与通用行为准则                        | `CLAUDE.md` 指向 `AGENTS.md`，不维护第二份正文；细则就近拆分并从 `AGENTS.md` 链接      |
...
| pages-api 内部架构                | `apps/pages-api/README.md`                                                                                                                                | pages-api 分层、依赖方向、Store/Provider 边界和变更影响面                          | pages-api 代码边界、依赖或验证方式变化时同步；公开 API 合约仍以 OpenAPI 源码为准        |
...
| legacy API / Site Publishing 退休 | `docs/operations/legacy-api-and-site-publishing-retirement.md`                                                                                            | v1 410 协议、Site Publishing 冻结、资源与数据保留、人工上线和回滚顺序             | 不把退休误写成资源删除或 v1 到 v2 迁移                                                 |
...
| Slack Review 结果摘要             | `docs/architecture/slack-review-results-summary.md`                                                                                                       | Slack 中查看 Review Agent 评论摘要、blocker、建议和 site-check 结论的只读能力规格 | Review 结果查询 intent / toolCall、Review gate 展示或 Slack 诊断能力变更时同步         |
...
`apps/server` 属于 v1 legacy，代码已进入墓碑模式：只保留精确的 `GET/HEAD /health`，其它请求统一返回 `410 LEGACY_API_RETIRED`。旧站点、Worker、route、DNS、Custom Domain、KV 和 metadata 继续保留，退休不包含 Cloudflare 资源物理删除，也不包含 v1 到 v2 迁移。v2 新建站点默认也使用 `workers.xd.team` 子站后缀，但由 `apps/pages-api`、`apps/pages-router` 和 v2 route/hostname claim 体系承载；根 README、agent 规范和 API 边界文档默认描述 v2。退休边界和人工上线顺序以 [legacy API 与 Site Publishing 退休手册](./operations/legacy-api-and-site-publishing-retirement.md) 为准。

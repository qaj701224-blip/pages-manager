...
| v2 架构 | `docs/pages-v2-wfp-architecture.md` | XD Pages v2 架构索引 | 正文按 `docs/architecture/`、`docs/operations/`、`docs/security/` 拆分 |
| v2 API 开发合约 | `apps/pages-api/src/openapi.js` | pages-api 开发、测试和受控内部集成合约 | 不公开 `/openapi.json`；用户入口是 CLI |
| 用户 / agent 操作 | `apps/pages-skill/skill/SKILL.md`、CLI help | 发布、状态、访问控制和回滚流程 | 不手写部署 HTTP 请求，不复刻内部 API |
| runtime helper / SDK | `apps/pages-sdk/README.md`、`apps/pages-skill/skill/references/sdk.md` | `@xd/pages-sdk` 用法和 agent 侧 SDK 指引 | SDK 行为变更时同步 package README 与 skill reference |
...
| 架构决策记录 | `docs/adr/` | 长期设计决策和取舍 | ADR 过长时保留索引页，正文拆到同名目录 |
...
1. [XD Pages v2 架构索引](./pages-v2-wfp-architecture.md)

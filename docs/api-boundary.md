...
`owner.displayName` 只提供个人真实姓名、custom team 名称或 department team 派生的安全叶名称；缺少安全展示名时返回 `null`，不回退到邮箱、内部 ID，也不返回完整部门路径。`owner.isCurrentUser` 只表示当前认证用户是该站点的个人直接 Owner，team-owned 站点始终为 `false`。`permissions.canDeploy` 是当前请求凭证此刻能否部署该既有站点的 point-in-time 提示，不能作为授权凭据；部署请求仍会重新读取权威状态并重新鉴权。

...
每次 patch 或命令执行后，executor 会重新扫描当前改动路径和文件内容。发现 forbidden path、高风险未批准路径或 secret-looking 内容时，工具 observation 会把错误返回给模型；模型可以继续修复。`finish` 时仍会做最终校验。workflow 在提交前还会按包含未跟踪文件的 changed-file 列表逐个读取内容做 secret scan。workflow 自身写入的 `.pages-artifacts/**` 只作为 callback / report 临时目录，不参与 executor 和 workflow 的最终 changed-file 判断，也不会被 secret scan 或 commit 纳入；`.pages-trusted/**` 同样只用于可信 helper checkout，不参与目标仓库 diff。

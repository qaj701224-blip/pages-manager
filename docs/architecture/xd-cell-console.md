...
- `设置`：名称与站点 URL 使用两个独立表单和错误状态；名称可清空，URL 改名的 `202 pending` 会自动轮询至 ready，旧地址停止访问并在安全期后释放，同时提示同步本地 `xd-cell.config.json.name`。运行配置保存或替换后保留现有列表并后台刷新，弹窗开关不应造成页面横向位移。删除站点必须二次确认；站点归属转移单独设计，不和普通表单保存混在一起。
...
| `publisher` | 站点管理者 | 创建、发布和更新团队站点，修改名称/URL、访问控制和运行配置，删除站点、转移站点归属    |

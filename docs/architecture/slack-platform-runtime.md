...
- Slack retry / follow-up 把任务推进到 running 类状态后，如果 worker dispatch 返回未启动或抛错，gateway 必须立刻把任务落到 `failed` 并回写可见状态，不能留下没有 worker 消费的 `generating_page` / `fixing`。
...
- Slack Interactivity 确认类按钮在写入 MySQL 后必须快速 ACK；worker 启动放入 background / waitUntil 执行，避免 GitHub 或 Actions dispatch 慢调用占用 Slack 的交互响应窗口。direct `issue:` / `site:` 测试入口如果同步启动 worker 被拒绝，必须把 delivery 和 job 标成失败，不能留下 `received`。
...
- GitHub issue / PR 被 reopen 后，无论触发源是 GitHub webhook、Slack 按钮还是 Slack Agent tool，如果恢复任务时 worker dispatch 被拒绝或抛错，必须把 job / work item 记录为 `worker_start_failed`，不能只 ACK 后留下运行中状态。
...
merge 频道公告：GitHub merge webhook 后的系统级富文本摘要
```

...
- Slack 运行入口、session 和数据模型写在 [slack-platform-runtime.md](./slack-platform-runtime.md)；Agent turn 协议、语义分块准流式回复和进度消息 / message binding 细节写在 [slack-agent-runtime.md](./slack-agent-runtime.md)；Slack Agent prompt package 和 policy skill 落地规格写在 [slack-agent-policy-skill.md](./slack-agent-policy-skill.md)；repo 只读问答边界写在 [slack-agent-repo-question.md](./slack-agent-repo-question.md)。

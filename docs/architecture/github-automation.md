...
- 通过摘要里的 `0 failed`、`failed: 0`、`0 errors`、`errors: 0`、`no security issues` 或 `no critical issues` 不视为 blocking；真实非零 failed / error 计数和明确 security / critical 风险仍视为 blocking。

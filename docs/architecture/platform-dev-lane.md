...
自动修复、人工 gate 放行后 dispatch Platform Agent 时，如果 worker start 返回失败，item 必须落到 `failed` 并记录可见错误；GitHub webhook 驱动的 site preview / Pages Agent dispatch 则必须让 delivery 保持可重试，不能把未启动的 worker 当成已处理。

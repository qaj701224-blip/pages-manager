...
test('deployment completion preserves committed success when terminal persistence fails', async () => {
  ...
  const failures = [];
  const stage = { operation: 'persist_succeeded_deployment' };
  const application = createApplication({
    deployments: {
      update: async () => {
        throw cause;
...
test('deployment completion records trace success after persistence', async () => {
  ...
  const application = createApplication({
    ...
    telemetry: {
      startPersist(operation) {
        calls.push(['trace_start', operation]);
        return operation;
      },
      async persistSucceeded(stage) {
        calls.push(['trace_succeeded', stage]);
      },
  ...
  assert.equal(await application.complete(command), completed);
  assert.deepEqual(calls, [
    ['trace_start', 'persist_succeeded_deployment'],
    'update',
    ['trace_succeeded', 'persist_succeeded_deployment'],
  ]);
});
...
test('deployment completion starts persistence telemetry synchronously', () => {
  const startError = new Error('invalid trace');
  const application = createApplication({
    deployments: { update: async () => assert.fail('deployment update must not run') },
    telemetry: {
      startPersist() {
        throw startError;
      },
      persistSucceeded: async () => assert.fail('success trace must not run'),
      persistFailed: async () => assert.fail('failure trace must not run'),
    },
  });

  assert.throws(() => application.complete(command), (error) => error === startError);
});

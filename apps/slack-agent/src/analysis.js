...
function redactSecretLikeText(text = '') {
  return String(text || '')
    ...
    .replace(jsonSecretFieldPattern, (_, prefix, value) => `${prefix}${value[0]}[REDACTED_SECRET]${value[0]}`)
    .replace(quotedSecretAssignmentPattern, (_, key, operator, value) => {
      const quote = value[0];

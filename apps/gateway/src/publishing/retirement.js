...
export function sitePublishingRetiredIgnoredResponse(extra = {}) {
  return jsonResponse({
    ok: true,
    ignored: SITE_PUBLISHING_RETIRED_CODE,
    message: SITE_PUBLISHING_RETIRED_MESSAGE,
    ...extra,
  });
}

export function isSitePublishingWorkItem(item = {}) {
  return Boolean(item?.id) && item.workItemKind !== 'platform_dev';
}

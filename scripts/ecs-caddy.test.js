...
test('ECS worker exposes platform workflow and base refs', () => {
  const compose = readRepoFile('docker-compose.ecs.yml');
  const envExample = readRepoFile('.env.ecs.example');

  assert.match(compose, /pages-worker:[\s\S]*PAGES_PLATFORM_WORKFLOW_REF: \$\{PAGES_PLATFORM_WORKFLOW_REF:-master\}/);
  assert.match(
    compose,
    /pages-worker:[\s\S]*PAGES_PLATFORM_BASE_REF: \$\{PAGES_PLATFORM_BASE_REF:-\$\{PAGES_PLATFORM_PR_BASE_REF:-master\}\}/
  );

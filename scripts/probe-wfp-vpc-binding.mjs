...
// staging dispatch namespace 做「带 / 不带 vpc binding」的对照上传,把 Cloudflare 的原始(已脱敏)响应
// 打印出来,用来区分两种根因:CF_API_TOKEN 缺 Connectivity Directory 权限,还是 binding metadata 格式问题。
//
// 安全约束:
//   - 只操作 staging dispatch namespace,显式拒绝 production;
//   - 临时脚本名带 vpc-probe- 前缀 + 时间戳,每次探测跑完立即 DELETE;
...
//     node scripts/probe-wfp-vpc-binding.mjs [dispatch-namespace]
...
const dispatchNamespace = (process.argv[2] || 'xd-cell-workers-staging').trim();

if (dispatchNamespace.includes('production')) {
  console.error('::error::refusing to probe a production dispatch namespace');
  process.exit(2);
}
...
if (baseline.ok && !vpc.ok) {
  ...
  console.log('\n结论:staging 下带 vpc_network binding 也能成功。说明 staging 的 CF_API_TOKEN 权限没问题;');
  console.log('朱峰的失败集中在 production 侧,需用同一脚本在 production 侧(换对应 token/tunnel)复现对照。');
}
